...
# Initialize session state for tracking if the button was clicked
if "run_quality_folding" not in st.session_state:
    st.session_state.run_quality_folding = False

# UI: Button to start Quality Based Folding
st.title("Labeling")
...
if not st.session_state.run_quality_folding:
    if st.button("Run Quality Based Folding"):
        ...
        st.rerun()  # Refresh to show the HTML


# If button was clicked, show the HTML content
if st.session_state.run_quality_folding:
    ...
    if st.button("Next"):
      st.switch_page("pages/DetectedErrors.py")
